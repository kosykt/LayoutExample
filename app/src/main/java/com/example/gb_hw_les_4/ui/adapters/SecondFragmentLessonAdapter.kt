...
class SecondFragmentLessonAdapter(
    val zoom: (Lesson) -> Unit,
) : ListAdapter<Lesson, SecondFragmentLessonAdapter.LessonViewHolder>(LessonCallback) {

    inner class LessonViewHolder(private val vb: ItemClassesBinding) :
        RecyclerView.ViewHolder(vb.root) {
        fun show(lesson: Lesson) {
            vb.itemClassesTopCardTimeTextView.text = lesson.time
            vb.itemClassesTopCardTimeTextView.visibility = View.VISIBLE
            vb.itemClassesLessonLabel.text = lesson.lesson
            vb.itemLessonGroupTime.visibility = View.GONE
            vb.itemClassesTeacher.visibility = View.VISIBLE
            vb.itemClassesTeacher.text = lesson.teacher
            if (lesson.canOpenIn) {
            ...
            if (lesson.description.isNotEmpty()) {
                vb.itemClassesDescription.text = lesson.description
                vb.itemClassesDescription.visibility = View.VISIBLE
